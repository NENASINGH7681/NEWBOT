...
def sanitize_filename(name):
    """Improved sanitization for Windows filenames"""
    if not name:
        return "unnamed_file"
...
def get_message_file_size(msg):
    """Get file size from different message types"""
    if msg.document:
        ...
        if 't.me/c/' in msg_link or 't.me/b/' in msg_link:
            ...
            if chat in saved_channel_ids:
                await app.edit_message_text(
                    ...
                    "Sorry! This channel is protected by **__KINGSTON__**."
                )
                return
...
async def get_media_filename(msg):
    if msg.document:
        return sanitize_filename(msg.document.file_name) if msg.document.file_name else "unnamed_file"
    if msg.video:
        return sanitize_filename(msg.video.file_name) if msg.video.file_name else "video.mp4"
    if msg.photo:
        return "photo.jpg"
    return "unnamed_file"
...
get_user_rename_preference = lambda user_id: user_rename_preferences.get(str(user_id), 'KINGSTON')
...
async def send_settings_message(chat_id, user_id):
    ...
    buttons = [
        ...
        [Button.url("Report Errors", "https://t.me/KINGSTONJK7")]
    ]
...
async def callback_query_handler(event):
    ...
    if event.data == b'setchat':
        ...
        await event.edit("Choose your preferred upload method:\n\n__**Note:** **SpyLib ⚡**, built on Telethon(base), by KINGSTON still in beta.__", buttons=buttons)
    ...
    try:
        ...
        if freecheck == 1:
            reply_markup = InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("💎 Get Premium to Forward", url="https://t.me/KINGSTONJK7")]
                ]
            )
...
async def rename_file(file, sender):
    ...
    last_dot_index = original_name.rfind('.')
    
    if last_dot_index > 0:  # Only if there's an extension
        original_file_name = original_name[:last_dot_index]
        file_extension = original_name[last_dot_index + 1:]
    ...
    # Apply word replacements and deletions
    for word in delete_words:
    ...
    # Create new filename with custom tag
    ...
    try:
        ...
        return file  # Return original if rename fails

async def sanitize(file_name: str) -> str:
    """Wrapper for the sanitize_filename function"""
